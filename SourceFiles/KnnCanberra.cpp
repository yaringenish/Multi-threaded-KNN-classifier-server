...
/**
  ...
  */
KnnCanberra::KnnCanberra(std:: vector<double> inputVector,std:: vector<std::pair< std::string,
        std:: vector<double>>> vectors, int k,int vectorSize): Knn(inputVector,vectors,k,vectorSize){}
/**
  * This method calculates the distance between the vectors using the Canberra calculation.
  * @param v1 std:: vector<double> type
  * @param v2 std:: vector<double> type
  * @return the result of the calculation, double type.
  */
double KnnCanberra::compute(std::vector<double> &v1, std::vector<double> &v2){
    double sum = 0;
    int length = v1.size();
    for (int i = 0; i < length; i++) {
        double up = std::abs(v1.at(i) - v2.at(i));
        double down = std::abs(v1.at(i)) + std::abs(v2.at(i));
        if(down != 0) {
 ...
 */
bool KnnCanberra::greater(std::pair <std::string , std::vector<double>> p1 ,std::pair<std::string ,std::vector<double>> p2){
 ...
 */
std::string KnnCanberra::mostTimes () {
...
/**
  ...
  */
std::string KnnCanberra::printResult(){
