...
#define p 2.0
/**
 * Constructor method to initiate the KnnMinkowski object.
 * @param inputVector std:: vector<double> type
 * @param vectors std:: vector<double> type
 * @param k int type
 * @param vectorSize int type
 */
KnnMinkowski::KnnMinkowski(std:: vector<double> inputVector,std:: vector<std::pair< std::string,
        std:: vector<double>>> vectors, int k,int vectorSize): Knn(inputVector,vectors,k,vectorSize){}
/**
 * This method calculates the distance using the Minkowski calculation.
 * @param v1 std:: vector<double> type
 * @param v2 std:: vector<double> type
 * @return the result of the calculation, double type.
 */
double KnnMinkowski::compute(std::vector<double> &v1, std::vector<double> &v2){
    double sum = 0;
    int length = v1.size();
    for (int i = 0; i < length; i++) {
        double val = std::abs(v1.at(i) - v2.at(i));
        double temp = std::pow(val, p);
        sum += temp;
    }
    double result = std::pow(sum, 1 / p);
    return result;
}
/**
...
*/
bool KnnMinkowski::greater(std::pair <std::string , std::vector<double>> p1 ,std::pair<std::string ,std::vector<double>> p2){
 ...
 */
std::string KnnMinkowski::mostTimes () {
 ...
 */
std::string KnnMinkowski::printResult(){
