...
/**
  ...
  */
KnnEuclidean::KnnEuclidean(std:: vector<double> inputVector,std:: vector<std::pair< std::string,
 ...
 */
double KnnEuclidean::compute(std::vector<double> &v1, std::vector<double> &v2){
    double dist = 0;
    int length = v1.size();
    for (int i = 0; i < length; i++) {
        double temp = std::pow(abs(v1.at(i) - v2.at(i)), 2);
        dist = dist + temp;
    }
    dist = std::sqrt(dist);
    return dist;
 ...
 */
bool KnnEuclidean::greater(std::pair <std::string , std::vector<double> > p1 ,std::pair<std::string ,
 ...
 */
std::string KnnEuclidean::mostTimes () {
 ...
 */
std::string KnnEuclidean::printResult(){
